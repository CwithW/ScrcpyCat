  ...
  <div class="guest-config-overlay" @click.self="$emit('close')">
    <div class="guest-config-modal">
      ...
      <div class="gc-body">
        ...
        <div class="gc-section-title">设备运维能力</div>
        <label class="gc-toggle-row" v-for="permission in operationPermissions" :key="permission.key">
          <input type="checkbox" v-model="permission.allow.value" />
          <span>{{ permission.label }}</span>
        </label>
        <div class="gc-hint">Shell、文件库与批量任务均要求设备已分配；未勾选时前端入口隐藏，服务端同时拒绝请求。</div>
...
const operationPermissions = [
  { key: 'shell', label: '允许 Shell 与交互终端', allow: ref(!!props.user.permissions?.shell) },
  { key: 'file_library', label: '允许服务器文件库', allow: ref(!!props.user.permissions?.file_library) },
  { key: 'batch_tasks', label: '允许批量任务', allow: ref(!!props.user.permissions?.batch_tasks) }
]
...
async function save() {
  ...
  try {
    const body = {
      ...
      permissions: {
        shell: operationPermissions[0].allow.value,
        file_library: operationPermissions[1].allow.value,
        batch_tasks: operationPermissions[2].allow.value
      },
