          ...
          <div v-if="activeTab === 'audio'" class="tab-pane">
            <div class="form-group form-group-row">
              <div class="group-info">
                ...
                <small v-if="audioUnavailableReason" class="hint">{{ audioUnavailableReason }}</small>
                <small v-else class="hint">回传云手机声音 (开启后默认设为 output)</small>
              </div>
              <div class="toggle-switch">
                <input type="checkbox" id="audio-toggle" v-model="localSettings.audio" :disabled="isLocked('audio') || !!audioUnavailableReason" />
            ...
            <div v-if="localSettings.audio" class="sub-section animate-slide-down">
              <div class="form-group">
                ...
                <select v-model="localSettings.audioSource" class="select-input" :disabled="isLocked('audio') || !!audioUnavailableReason">
                  <option value="output">output (捕获扬声器声音，物理手机静音)</option>
                  <option value="playback">playback (应用内录，限 Android 13+)</option>
                ...
                <small class="hint" v-else-if="localSettings.audioSource === 'playback'">仅能捕获允许内录的 App 音频；下方开关控制是否保留手机本地声音</small>
              ...
              <div class="form-group form-group-row" v-if="localSettings.audioSource === 'playback'">
                ...
                <div class="toggle-switch">
                  <input type="checkbox" id="audio-dup-toggle" v-model="localSettings.audioDup" :disabled="isLocked('audio') || !!audioUnavailableReason" />
              ...
              <div class="form-group">
                <label>音量增益</label>
                <input type="number" v-model.number="localSettings.audioGain" min="0.5" max="3" step="0.25" :disabled="isLocked('audio') || !!audioUnavailableReason" />
