    ...
    <div class="sidebar-panel">
      ...
      <div class="device-list-container">
        <div v-if="onlineDevices.length === 0" class="no-devices">
          当前没有支持外设注入的在线设备。此功能需要设备端的定制 GPS / 传感器服务。
        </div>
        <div 
...
const onlineDevices = computed(() => {
  return deviceStore.devices.filter(d => (d.status === 'online' || d.status === 'registered') && d.info?.capabilities?.[activeTab.value === 'gps' ? 'gps_injection' : 'sensor_injection'] === true)
})
...
const hasTargets = computed(() => {
  ...
  return onlineDevices.value.some(d => d.id === selectedDeviceId.value)
})
...
function injectData(channel, payload) {
  ...
  const tIds = (batchEnabled.value ? batchDeviceIds.value : [selectedDeviceId.value]).filter(id => onlineDevices.value.some(d => d.id === id))
