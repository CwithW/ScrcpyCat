        ...
        <div v-if="activeTab === 'files'" class="tab-pane">
          <div class="task-form-card">
            ...
            <button 
              ...
              :disabled="selectedDeviceIds.length === 0 || !uploadedFileName || uploadProgress < 100 || isSubmitting"
                  ...
                  <td>
                    <div class="progress-cell">
                      <div class="progress-track">
                        <div class="progress-fill" :style="{ width: (isTaskFinished(sub.status) ? 100 : 0) + '%' }"></div>
                      </div>
                      <span>{{ isTaskFinished(sub.status) ? '100%' : '—' }}</span>
                    </div>
                  </td>
                  <td class="log-cell">
                    <span v-if="isTaskFinished(sub.status) && sub.status !== 'success'" class="error-text" :title="sub.result">
                      {{ sub.result || '未知错误' }}
                    </span>
                    <span v-else-if="sub.status === 'success' && sub.result" class="success-log" @click="showLogModal(sub.device_id, sub.result)">
                      点击查看输出结果
                    </span>
                    <span v-else>-</span>
                  </td>
...
].filter(tab => tab.value !== 'files' || authStore.canFileLibrary))
const activeTab = ref(authStore.canFileLibrary ? 'files' : 'apps')
watch(tabs, items => {
  if (!items.some(tab => tab.value === activeTab.value)) activeTab.value = items[0]?.value || 'apps'
})
...
const fetchCloudFiles = async () => {
  if (!authStore.canFileLibrary) return
  try {
...
const failedCount = computed(() => {
  if (!currentTask.value) return 0
  return Object.values(currentTask.value.devices).filter(sub => isTaskFinished(sub.status) && sub.status !== 'success').length
})
const finishedCount = computed(() => {
  if (!currentTask.value) return 0
  return Object.values(currentTask.value.devices).filter(sub => isTaskFinished(sub.status)).length
})
