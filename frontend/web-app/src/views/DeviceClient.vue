  ...
  <div class="device-panel-view" :class="{ 'is-mobile': isMobile, 'mobile-landscape': isMobile && isVideoLandscape, 'is-web-fullscreen': isWebFullscreen, 'is-camera-surveillance': isCameraMode }" @mousemove="onFsUiActivity" @touchstart.passive="onFsUiActivity">
    ...
    <div class="device-client-main">
      ...
      <div class="video-wrapper" ref="containerRef">
        ...
        <div v-if="currentWebRTC.audioError?.value" class="audio-error-notice" role="status">
          {{ currentWebRTC.audioError.value }}
          <button v-if="isWebSocketMode" @click.stop="togglePageMute">重试声音</button>
        </div>
        ...
        <div v-if="showOverlay" class="panel-overlay">
          <div class="overlay-box">
            ...
            <template v-else-if="currentWebRTC.error.value">
              ...
              <template v-if="!isWebSocketMode && !isCameraMode">
        ...
        <div v-if="isMobile || isFullscreen || isWebFullscreen" class="mobile-fab-container" :style="fabStyle">
          ...
          <div class="mobile-fab-menu" :class="{ 'show': showMobileMenu, 'align-left': isFabOnLeft, 'align-top': isFabOnTop }">
            ...
            <template v-if="isCameraMode">
              ...
              <button class="fab-item" :disabled="!!audioUnavailableReason" :title="audioUnavailableReason || pageAudioAction" @click="togglePageMute(); showMobileMenu=false">
                {{ pageAudioAction }}
              </button>
              <div class="fab-divider"></div>
            ...
            <template v-else>
              ...
              <button class="fab-item" :class="{ 'group-active': isWebSocketMode }" :disabled="isWebSocketMode && !webrtcAvailable" :title="streamModeTitle" @click="toggleStreamMode(); showMobileMenu=false">
              ...
              <button class="fab-item" :disabled="!!audioUnavailableReason" :title="audioUnavailableReason || pageAudioAction" @click="togglePageMute(); showMobileMenu=false">
                <svg v-if="pageAudioOff" class="icon" viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line></svg>
    ...
    <div v-if="!isMobile && !isMini && !isCameraMode" class="control-sidebar" :class="{ 'fs-ui-visible': fsUiVisible }">
      <div class="sidebar-group">
        ...
        <button class="sidebar-btn" :class="{ active: isWebSocketMode }" @click="toggleStreamMode" :title="streamModeTitle" :disabled="isWebSocketMode && !webrtcAvailable">
          <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <button class="sidebar-btn" :class="{ active: localSettings.audio && !pageAudioOff }" :disabled="!!audioUnavailableReason" @click="togglePageMute" :title="audioUnavailableReason || pageAudioAction">
          <svg v-if="pageAudioOff" class="icon" viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line></svg>
          <svg v-else class="icon" viewBox="0 0 24 24"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15 9a5 5 0 0 1 0 6"></path><path d="M17.7 6.3a9 9 0 0 1 0 11.4"></path></svg>
          <span class="btn-text">{{ pageAudioAction }}</span>
        </button>
        ...
        <button class="sidebar-btn" @click="takeScreenshot" title="截图">
          <svg class="icon" viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2"></rect><circle cx="12" cy="12" r="4"></circle></svg>
    ...
    <div v-if="!isMobile && !isMini && isCameraMode" class="camera-sidebar">
      ...
      <div class="camera-sidebar-scroll">
        ...
        <div class="cam-section">
          <div class="cam-section-header">
            <span>环境声音监听</span>
            <span class="cam-badge" :class="{ 'audio-on': localSettings.audio && !pageAudioMuted }">{{ !localSettings.audio ? '未开启' : (pageAudioMuted ? '已静音' : '监听中') }}</span>
          </div>
          <div class="audio-control-row">
            <button class="cam-btn audio-toggle-btn" :class="{ active: localSettings.audio && !pageAudioMuted }" :disabled="!!audioUnavailableReason" :title="audioUnavailableReason" @click="togglePageMute">
              ...
              <span>{{ !localSettings.audio || pageAudioMuted ? '开启声音监听' : '关闭声音监听' }}</span>
            </button>
          </div>
        </div>
    ...
    <div v-if="showClipboardText" class="clipboard-overlay" @click.self="showClipboardText = false">
      <section class="clipboard-dialog">
        <h3>设备剪贴板</h3>
        <textarea :value="clipboardText" readonly @focus="$event.target.select()"></textarea>
        <div><button @click="copyDeviceClipboard">复制</button><button @click="showClipboardText = false">关闭</button></div>
      </section>
    </div>
    <!-- 截图预览弹窗 -->
    <ScreenshotModal 
    ...
    <SettingsModal 
      ...
      :audio-unavailable-reason="audioUnavailableReason"
...
import { isWebRTCAvailable, WEBRTC_UNAVAILABLE_MESSAGE } from '@/utils/webrtc'
...
function isLensActive(lens) {
  ...
  // 同方向的物理镜头必须按 ID 区分；未指定 ID 时使用该方向的首个镜头。
  const selectedId = localSettings.value.cameraId ||
    availableLenses.value.find(candidate => candidate.facing === lens.facing)?.id || ''
  if (String(selectedId) !== String(lens.id)) return false
  const currentZoom = localSettings.value.cameraZoomRatio || 1.0
  return Math.abs(currentZoom - (lens.zoom || 1.0)) < 0.15
}
...
const deviceBatteryText = computed(() => {
  const stats = currentDevice.value?.metrics
  if (!stats) return ''
  let text = ''
  if (stats.battery !== undefined && stats.battery >= 0) {
    text += `🔋 ${stats.battery}%`
  }
...
function saveSettings(newSettings) {
  ...
  // 视频和音频设置共用连接生命周期，与手动“关闭面板再点连接”同效：先卸载面板触发完整断开清理（onUnmounted → disconnect），
...
function createStreamInstance(deviceId, options) {
  ...
  if (mode === 'websocket' || (!isWebRTCAvailable() && options.video_source !== 'camera')) {
...
const streamModeTitle = computed(() => {
  if (isWebSocketMode.value && !webrtcAvailable) return '浏览器未启用 WebRTC，已使用 WebSocket 投屏'
  return isWebSocketMode.value ? '当前为 WebSocket 投屏，点击切换为 WebRTC 直连' : '当前为 WebRTC 直连，点击切换为 WebSocket 投屏'
})
const audioUnavailableReason = computed(() => {
  if (authStore.userPolicy?.forbid_audio && !localSettings.value.audio) return '管理员已关闭音频并锁定设置'
  if (isWebSocketMode.value) return isWebSocketAudioAvailable() ? '' : '浏览器不支持 Web Audio，无法播放声音'
  if (!webrtcAvailable) return WEBRTC_UNAVAILABLE_MESSAGE
  return ''
})
const pageAudioOff = computed(() => Boolean(audioUnavailableReason.value) || !localSettings.value.audio || effectiveAudioMuted.value)
const pageAudioAction = computed(() => {
  if (audioUnavailableReason.value) return '音频不可用'
  if (currentWebRTC.value?.audioError?.value) return '重试声音'
  if (!localSettings.value.audio) return '开启声音'
  return effectiveAudioMuted.value ? '取消静音' : '页面静音'
})

function toggleStreamMode() {
  if (isCameraMode.value || (isWebSocketMode.value && !isWebRTCAvailable())) return
...
function isPointInRenderedVideo(clientX, clientY) {
  const video = isCanvasRendering.value ? canvasElement.value : videoElement.value
...
function setupWebRTC() {
  ...
  webrtc.onClipboard((event) => {
    ...
    if (clipboardReadRequested) {
      clipboardReadRequested = false
      clipboardText.value = text
      showClipboardText.value = true
    }
    if (!text) {
      return
    }
...
function quickKey(cmd) {
  const key = /^input keyevent (\d+)$/.exec(cmd)
  if (key) {
    webrtc.sendInjectKeycode(0, Number(key[1]))
    setTimeout(() => webrtc.sendInjectKeycode(1, Number(key[1])), 30)
  } else {
...
async function copyDeviceClipboard() {
  if (navigator.clipboard?.writeText) {
    try { await navigator.clipboard.writeText(clipboardText.value); return } catch {}
  }
  const input = document.querySelector('.clipboard-dialog textarea')
  input?.select()
  document.execCommand('copy')
}

function togglePageMute() {
  if (audioUnavailableReason.value) return
  if (webrtc.audioError?.value && webrtc.retryAudio) {
    pageAudioMuted.value = false
    webrtc.retryAudio()
    return
  }
  if (props.audioMuted) {
  ...
  if (!localSettings.value.audio) {
    localSettings.value.audio = true
    pageAudioMuted.value = false
    reconnectStream(isCameraMode.value ? '正在开启环境声音监听...' : '正在开启设备声音...')
    return
  }
...
<style scoped>
.audio-error-notice { position: absolute; bottom: 48px; left: 12px; right: 12px; z-index: 12; padding: 8px 12px; border-radius: 6px; background: #30251ee8; color: #fbbf24; font-size: 12px; }
.audio-error-notice button { margin-left: 10px; cursor: pointer; }

.clipboard-overlay { position: fixed; inset: 0; z-index: 10000; background: #0008; display: grid; place-items: center; }
.clipboard-dialog { width: min(540px, 90vw); padding: 20px; border-radius: 12px; background: var(--bg-surface, #222); }
.clipboard-dialog textarea { width: 100%; min-height: 180px; margin: 12px 0; color: inherit; background: var(--bg-primary, #111); }
.clipboard-dialog button { margin-right: 12px; padding: 8px 16px; cursor: pointer; }
