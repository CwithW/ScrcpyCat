  ...
  <div class="deploy-page">
    <div class="deploy-layout">
      ...
      <section class="form-section">
        ...
        <p v-if="!webUsbSupported" class="form-hint">当前 HTTP 页面可下载部署包；网页 USB 部署需要通过 HTTPS 或 localhost 访问。</p>
        ...
        <button
          class="deploy-btn"
          :disabled="isDeploying || !form.signalingUrl || !webUsbSupported"
          @click="startDeploy"
        >
      ...
      <div class="right-column">
        <DeploymentCredentials @copy="copyCommand" />
        <!-- 部署日志/进度 -->
          ...
          <div v-if="manualMode === 'adb'" class="manual-mode-block">
            ...
            <div class="manual-layout">
              <div class="manual-download-col">
                <h3 class="manual-subtitle">第一步：下载已配置的 ADB 部署包</h3>
                <div class="download-row">
                  <button @click="downloadPackage('adb')" :disabled="isPackaging" class="download-card-btn gold-card">
                    <div class="card-title">⚡ ADB 一键部署资源包 (ZIP)</div>
                    <div class="card-desc">包含全平台 Agent 二进制、scrcpy Server及一键脚本，解压即可通过 ADB 运行。</div>
                  </button>
                </div>
              </div>
                ...
                <div class="guide-steps">
                  <div class="guide-step-item">
                    ...
                    <div class="step-content">
                      <p>解压下载的 <code>scrcpycat-adb.zip</code> 并进入解压后的目录，然后执行下方对应系统的一键部署脚本命令：</p>
...
adb shell "ps -A | grep scrcpycat-agent"

# 查看 Agent 服务运行日志
adb shell "tail -n 100 /data/local/tmp/scrcpycat-agent.log"</pre>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div v-else-if="manualMode === 'magisk'" class="manual-mode-block">
            ...
            <div class="manual-layout">
              <div class="manual-download-col">
                ...
                <div class="download-row">
                  <button @click="downloadPackage('magisk')" :disabled="isPackaging" class="download-card-btn magisk-card">
                    <div class="card-title">📱 Magisk / KSU 刷机模块 (ZIP)</div>
                    <div class="card-desc">专属模块压缩包，内置全架构支持、守护进程以及 scrcpycatctl 控制台工具。</div>
                  </button>
                </div>
              </div>
              ...
              <div class="manual-guide-col">
                ...
                <div class="guide-steps">
                  <div class="guide-step-item">
                    <span class="step-num">1</span>
                    <div class="step-content">
                      <p><b>刷入模块包</b>：打开手机上的 Magisk / KernelSU 管理器，选择“从本地安装”并选中 <code>scrcpycat-magisk.zip</code>，刷入成功后<b>重启手机</b>。</p>
                    </div>
                  </div>
                  ...
                  <div class="guide-step-item">
                    ...
                    <div class="step-content">
                      <p><b>检查与重启 Agent</b>：</p>
                      <div class="deploy-script-tabs">
                        <div class="script-box-title" style="color: #a855f7;">模块管理命令 (手机终端 / ADB shell)</div>
                        <div class="code-container">
                      ...
                      <p style="margin-top: 10px; font-size: 12px; color: var(--text-secondary); line-height: 1.6;">
                        部署包已包含当前填写的设备 ID、信令地址与一次性注册令牌，请在 24 小时内完成首次启动。注册成功后使用设备身份自动重连。<br>
                        ICE 中转配置由服务端下发。修改接入地址后重新生成模块；已注册身份保存在 <code>/data/adb/scrcpycat</code>。
...
adb shell "su -c scrcpycatctl status"

# 查看日志
adb shell "su -c scrcpycatctl log"</pre>
                    </div>
                  </div>
                </div>
              </div>
...
import DeploymentCredentials from '@/components/DeploymentCredentials.vue'

const { isDeploying, deployStatus, deployProgress, deployError, deployLog, deployAgent } = useDeploy()
const logArea = ref(null)
const manualMode = ref('adb')
const isPackaging = ref(false)
const webUsbSupported = Boolean(window.isSecureContext && navigator.usb)
const form = reactive({ signalingUrl: '', deviceId: '' })
...
function currentStep() {
  ...
  return isDeploying.value ? 0 : -1
}
function stepClass(index) {
  return deployError.value && index === currentStep() ? 'error' : index < currentStep() ? 'done' : index === currentStep() ? 'active' : ''
}
function stepIcon(index) { return ({ error: '✗', done: '✓', active: '⟳' })[stepClass(index)] || '○' }
watch(deployLog, async () => { await nextTick(); if (logArea.value) logArea.value.scrollTop = logArea.value.scrollHeight }, { deep: true })
async function startDeploy() { await deployAgent({ ...form }) }
const shCommand = computed(() => './run.sh')
const batCommand = computed(() => 'run.bat')
const magiskCommand = computed(() => 'su -c scrcpycatctl status\nsu -c scrcpycatctl restart')

async function downloadPackage(mode) {
  isPackaging.value = true
  try {
    const response = await fetch('/api/deploy/package', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + (localStorage.getItem('auth_token') || '') },
      body: JSON.stringify({ mode, device_id: form.deviceId, signaling: form.signalingUrl })
    })
    if (!response.ok) { const data = await response.json(); throw new Error(data.error || '生成部署包失败') }
    const url = URL.createObjectURL(await response.blob())
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = 'scrcpycat-' + mode + '.zip'
    document.body.appendChild(anchor)
    anchor.click()
    anchor.remove()
    setTimeout(() => URL.revokeObjectURL(url), 60000)
  } catch (error) { alert(error.message) } finally { isPackaging.value = false }
}
async function copyCommand(text) {
  try {
    if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(text)
      ...
      input.remove()
      if (!copied) throw new Error('请手动选择复制')
    }
  } catch (error) { alert(error.message) }
}
onMounted(() => { form.signalingUrl = (location.protocol === 'https:' ? 'wss://' : 'ws://') + location.host })
</script>
