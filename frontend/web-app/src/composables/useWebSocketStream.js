...
export function useWebSocketStream(deviceId, options = {}) {
  ...
  function prepareAudio() {
    ...
    if (!isWebSocketAudioAvailable()) {
      audioError.value = '浏览器不支持 Web Audio，无法播放声音'
      return
    }
    try {
      audioPlayer = new WebSocketAudioPlayer({
        muted: audioMuted.value,
        gain: options.audio_gain,
        onError: message => { audioError.value = message },
        onPlaying: () => { clearTimeout(audioTimeout); audioTimeout = null }
      })
    } catch (failure) {
      audioError.value = failure.message || '音频初始化失败'
    }
  }
  ...
  signaling.onAudioFrame(packet => {
    if (!audioPlayer || audioPlayer.closed) return
    // 收到音频即证明设备在推流；浏览器等待用户手势时不应误报设备超时。
    clearTimeout(audioTimeout)
    audioTimeout = null
    audioPlayer.feed(packet)
  })
  watch(signaling.signalingReady, ready => { if (ready) requestAudio() })
  watch(audioError, message => {
    if (message) {
      clearTimeout(audioTimeout)
      signaling.stopAudioForwarding()
      audioPlayer?.close()
    }
  })
  watch(signaling.status, state => {
    if (options.audio && status.value === 'connected' && (state === 'error' || state === 'disconnected')) {
      audioError.value = '音频连接已断开，请重新连接设备'
    }
  })
  ...
  function renderYUV(canvasEl, yuv, width, height) {
    const size = drawYUV420(canvasEl, yuv, width, height, frameCrop)
    if (!size) return
    frameCount++
    if (videoNaturalSize.value.width !== size.width || videoNaturalSize.value.height !== size.height) {
      videoNaturalSize.value = size
      if (frameSizeCallback) frameSizeCallback(size.width, size.height)
    }
    if (!isFirstFrameRendered.value) {
      isFirstFrameRendered.value = true
      status.value = 'connected'
  ...
  const sendCommand = signaling.sendCommand
  const setClipboard = signaling.setClipboard
  const getClipboard = signaling.getClipboard
  const sendInjectText = sendText
  function requestScreenshot() { signaling.requestScreenshot() }
  ...
  return {
    ...
    signalingReady: signaling.signalingReady,
    createAdbSessionChannel: signaling.createAdbSessionChannel,
    onCommandResult: signaling.onCommandResult,
    executeCommand: signaling.executeCommand,
    sendInjectText,
    requestScreenshot,
    status,
    error,
