...
import { openTerminalSession } from '@/utils/terminalSession'
import { isWebRTCAvailable, WEBRTC_UNAVAILABLE_MESSAGE } from '@/utils/webrtc'
import { parseAudioFrame } from '@/utils/websocketAudio'
...
export function useWebRTC(deviceId, options = {}) {
  ...
  // Agent 还会执行控制平面下发的可信权限，防止绕过界面直接发送输入。
  ...
  async function connect(shareTokenParam = null, sharePwdParam = '') {
    ...
    if (!options.signalingOnly && !isWebRTCAvailable()) {
      failConnection(WEBRTC_UNAVAILABLE_MESSAGE)
      return
    }
    ...
    let ticket
    try {
      ticket = await issueWebSocketTicket(shareTokenParam || '', sharePwdParam)
    } catch (err) {
      failConnection(err.message)
      return
    }
    const wsUrl = `${wsProtocol}//${location.host}/connect_client?ticket=${encodeURIComponent(ticket)}`
    
    debugLog('[Signaling] Connecting to signaling service')
    ws = new WebSocket(wsUrl)
    ws.binaryType = 'arraybuffer'
    const signalingSocket = ws

    ws.onopen = () => {
      if (ws !== signalingSocket) return
    ...
    ws.onmessage = async (evt) => {
      ...
      if (evt.data instanceof ArrayBuffer) {
        const packet = parseAudioFrame(evt.data, deviceId)
        if (packet) audioFrameCallback?.(packet)
        return
      }
      ...
      try {
        ...
        if (ws !== signalingSocket) return
        console.error('[Signaling] Failed to handle message:', e)
        failConnection('连接协商失败：' + e.message)
      }
    }
  ...
  function handleMessage(msg) {
    ...
    switch (type) {
      case 'config':
        if (msg.device_info) handleDeviceInfo(msg.device_info)
        signalingReady.value = true
        ...
        if (options.signalingOnly) { status.value = 'connected'; break }
      ...
      case 'device_info':
        ...
        break
      case 'device_msg':
        return handleDeviceMessage(msg.payload)
      case 'command_result':
        handleCommandResult(msg)
        break
      case 'clipboard':
        if (options.signalingOnly || clipboardChannel?.readyState !== 'open') clipboardCallback?.(msg.payload)
        break
      case 'screenshot_response':
        if (msg.error) { error.value = msg.error; break }
      ...
      case 'pty_closed':
      case 'pty_error':
        terminalSessions.get(msg.session_id)?.handleMessage(msg)
        break
      case 'audio_error':
  ...
  function handleDeviceInfo(info) {
    if (info) {
      if (info.agent_version || info.app_version) {
        agentVersion.value = info.agent_version || info.app_version
      }
...
async function handleDeviceMessage(payload) {
  ...
  switch (payload.type) {
    case 'offer': {
      ...
      for (const candidate of pendingCandidates.splice(0)) {
        if (shouldKeepCandidate(candidate.candidate)) await peer.addIceCandidate(new RTCIceCandidate(candidate))
      }
    ...
    case 'ice-candidate':
      if (options.signalingOnly) return
      if (!isWebRTCAvailable()) throw new Error(WEBRTC_UNAVAILABLE_MESSAGE)
      if (!pc && payload.candidate) { pendingCandidates.push(payload.candidate); break }
      if (pc && payload.candidate) {
        ...
        if (shouldKeepCandidate(candStr)) {
          ...
          if (!pc.remoteDescription) pendingCandidates.push(payload.candidate)
          else pc.addIceCandidate(new RTCIceCandidate(payload.candidate))
  ...
  function playAudioTrack(track) {
    ...
    if ((gain > 1 || getAudioLowLatency()) && (window.AudioContext || window.webkitAudioContext)) {
      const AudioContextClass = window.AudioContext || window.webkitAudioContext
      audioContext = new AudioContextClass({ latencyHint: 'interactive' })
      audioSourceNode = audioContext.createMediaStreamSource(audioStream)
      audioGainNode = audioContext.createGain()
      audioGainNode.gain.value = audioMuted.value ? 0 : gain
      audioSourceNode.connect(audioGainNode).connect(audioContext.destination)
      const resume = () => audioContext?.resume().catch(() => {})
      resume()
      if (audioContext.state === 'suspended') {
        audioUnlockHandler = resume
  ...
  function createPeerConnection() {
    ...
    pc.ontrack = (evt) => {
      ...
      if (evt.track.kind === 'audio') {
        // 减少浏览器默认的音频预缓冲；不支持该提示的浏览器保留自身策略。
        if ('jitterBufferTarget' in evt.receiver) {
          try { evt.receiver.jitterBufferTarget = 40 } catch {}
        }
    ...
    function diagnoseConnectionFailure() {
      ...
      if (hasOnlyDockerOrLoopback && clientHasPhysicalLanIp) {
        advice = 'Agent 上报的容器地址无法从当前网络访问。请检查 Linux Agent 是否使用 host 网络，以及控制平面的 TURN 地址与中继端口是否可达。'
        return advice
      }
  ...
  function setAudioMuted(muted) {
    ...
    if (audioGainNode) audioGainNode.gain.value = audioMuted.value ? 0 : getAudioGain()
    if (audioElement) {
      audioElement.muted = audioMuted.value
    }
  }
  ...
  function startAudioForwarding(audioOptions) {
    audioError.value = null
    if (ws?.readyState !== WebSocket.OPEN) return false
    ws.send(JSON.stringify({ ...audioOptions, message_type: 'start_audio', device_id: deviceId, audio: true }))
    return true
  }

  function stopAudioForwarding() {
    if (ws?.readyState === WebSocket.OPEN) {
      ws.send(JSON.stringify({ message_type: 'stop_audio', device_id: deviceId }))
    }
  }
  ...
  function setClipboard(text, options = false) {
    ...
    if (ws?.readyState === WebSocket.OPEN) {
      sendInjectData('clipboard', { type: 'set_clipboard', text, paste: Boolean(normalized.paste) })
      return true
    }
    return false
  }
  ...
  function getClipboard() {
    ...
    if (ws?.readyState === WebSocket.OPEN) {
      sendInjectData('clipboard', { type: 'get_clipboard' })
      return true
    }
    return false
  }
  ...
  function createAdbSessionChannel(options) {
    return openTerminalSession({ peer: pc, socket: ws, deviceId, sessions: terminalSessions, ...options })
  }
